...
function moveAdvancedSectionsIntoSettings() {
  ...
  const labels = {
    ...
    ai: ["AI Insights", "Local insight engine and future AI space"],
...
function saveSolanaRpcUrl() {
  ...
  if (!/^https:\/\/.+/i.test(value)) {
    showToast("Paste a full https Solana RPC URL");
    return;
  }
...
function renderAiInsightsPage() {
  ...
  const sections = [
    ["Personal Budgeting", getUnifiedInsights()],
...
function renderDashboard() {
  ...
  dashboardAlerts.innerHTML = `
    ...
    <section class="overview-grid">
      ...
      <div class="overview-card insight-card">
        <div class="overview-card-head"><span>AI Insights</span><strong>${details.insights.length}</strong></div>
        <div class="overview-list">${details.insights.map((insight) => `<div class="overview-row compact"><span>${escapeHtml(insight)}</span></div>`).join("")}</div>
      </div>
    </section>
  ...
  if (network.mint === NETWORKS.solanaPyusd.mint) {
    try {
      ...
      lastError = message.includes("Failed to fetch")
        ? new Error("Local Solana balance service is not reachable. Restart the AllocaFi server, then refresh again.")
        : error;
    }
  }
...
function getAutoVaultPassword() {
  return localStorage.getItem(VAULT_AUTO_PASSWORD_KEY) || "";
}

function isAutoVaultEnabled() {
  return Boolean(getAutoVaultPassword());
}
...
function scheduleAutoVaultSnapshot() {
  if (suppressAutoVaultSnapshot || !isAutoVaultEnabled()) return;
...
function collectVaultData() {
  return {
    ...
    financeData,
    settings: {
      solanaRpcUrl: localStorage.getItem(SOLANA_RPC_URL_KEY) || "",
      walletConnectProjectId: localStorage.getItem(WALLETCONNECT_PROJECT_KEY) || "",
...
async function decryptVaultFile(vaultFile, password) {
  ...
  if (payload.app_name !== "AllocaFi" || payload.validation !== "allocafi-vault") {
    throw new Error("Vault data failed validation");
  }
  return payload;
}
...
async function renderVaultDashboard() {
  ...
  try {
    ...
    vaultStats.innerHTML = [
      ...
      ["Auto Snapshot", autoMeta ? new Date(autoMeta.createdAt).toLocaleTimeString() : "Off", autoMeta ? `Stored locally, ${formatBytes(autoMeta.encryptedSize)}` : "Local browser restore point"],
...
async function enableAutoVaultSnapshot() {
  ...
  const password = window.prompt("Create an Auto Snapshot password for this device. This is stored locally so AllocaFi can update snapshots automatically.");
  if (!password) return;
  ...
  localStorage.setItem(VAULT_AUTO_PASSWORD_KEY, password);
  await createAutoVaultSnapshot();
  renderVaultDashboard();
  showToast("Auto Snapshot enabled");
}
...
async function restoreAutoVaultSnapshot() {
  ...
  try {
    ...
    saveAddressBook();
    saveFinanceData();
    if (data.settings?.solanaRpcUrl) localStorage.setItem(SOLANA_RPC_URL_KEY, data.settings.solanaRpcUrl);
    if (data.settings?.walletConnectProjectId) localStorage.setItem(WALLETCONNECT_PROJECT_KEY, data.settings.walletConnectProjectId);
