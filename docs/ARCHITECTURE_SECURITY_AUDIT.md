...
AllocaFi is a vanilla HTML/CSS/JavaScript single page app served by a local Node server. The working product is still non-custodial: it reads public wallet data, lets users organize balances into Virtual Budget Accounts, and sends only through external wallet approval.
...
3. User creates or auto-generates Virtual Budget Accounts.
...
- Existing dashboard, wallets, budget accounts, Vault, and send flows remain intact.
